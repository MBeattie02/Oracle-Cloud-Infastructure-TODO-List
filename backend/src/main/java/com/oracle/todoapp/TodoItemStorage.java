...
class TodoItemStorage {
  ...
  List<TodoItem> all() {
    ...
    try (
        ...
        PreparedStatement pstmt = conn
            .prepareStatement("SELECT id, description, creation_ts, done FROM todoitem ORDER BY creation_ts DESC");
  ...
  TodoItem getById(int id) {
    ...
    try (
        Connection conn = pool.getConnection();
        PreparedStatement pstmt = conn
            .prepareStatement("SELECT id, description, creation_ts, done FROM todoitem WHERE id=?");) {
      pstmt.setInt(1, id);
  ...
  TodoItem save(TodoItem item) {
    ...
    try (
      ...
      if (idInt >= 0) {
        ...
        try (
            PreparedStatement pstmt = conn.prepareStatement("INSERT INTO todoitem (description) VALUES (?)",
                new String[] { "id" });) {
          pstmt.setString(1, item.getDescription());
